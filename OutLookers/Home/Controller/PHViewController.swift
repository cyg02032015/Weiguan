...
class PHViewController: YGBaseViewController {
    ...
    private var firstPan: UIPanGestureRecognizer!
    private var seconedPan: UIPanGestureRecognizer!
    ...
    func setupSubViews() {
        ...
        slidePageScrollView = TYSlidePageScrollView(frame: CGRect(origin: CGPointZero, size: CGSize(width: ScreenWidth, height: ScreenHeight)))
        ...
        firstPan = dynamicVC.tableView.panGestureRecognizer
        view.addGestureRecognizer(firstPan)
        ...
        seconedPan = talentVC.tableView.panGestureRecognizer
...
extension PHViewController: TYSlidePageScrollViewDataSource, TYSlidePageScrollViewDelegate {
    ...
    func slidePageScrollView(slidePageScrollView: TYSlidePageScrollView!, horizenScrollToPageIndex index: Int) {
        scrollTo = index
        if index == 0 {
            view.removeGestureRecognizer(seconedPan)
            view.addGestureRecognizer(firstPan)
        }else {
            view.removeGestureRecognizer(firstPan)
            view.addGestureRecognizer(seconedPan)
        }
    }
}
...
extension PHViewController {
    ...
    func followLabelClick() {
        guard header.follow.text != "--" else { return }
        let vc = FollowPeopleViewController()
        vc.userId = user
        vc.urlStr = API.hFollows
        vc.showType = .HFollow
        vc.title = "他的关注"
    ...
    func fansLabelClick() {
        guard header.fans.text != "--" else { return }
        let vc = FollowPeopleViewController()
        vc.userId = user
        vc.urlStr = API.hFans
        vc.showType = .Fan
